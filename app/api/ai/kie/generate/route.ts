...
// 启动时检查 API Key 是否加载成功
if (!KIE_API_KEY) {
  console.error("[Kie Generate] 警告: KIE_API_KEY 环境变量未设置，图片生成功能将不可用")
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { prompt, aspectRatio, resolution, webhookUrl, chatMessageId, userId, model, quality } = body
    ...
    const modelMap: { [key: string]: string } = {
      ...
      "gptImage1_5": "gpt-image/1.5-text-to-image",
      "gptImage2": "gpt-image-2-text-to-image",
      "seedream5Lite": "seedream/5-lite-text-to-image",
      "seedream5Pro": "seedream/5-pro-text-to-image",
    ...
    if (useWebhookMode && finalWebhookUrl) {
      ...
      if (!recordId) {
        ...
        console.log('生成记录创建成功（Webhook模式，待创建任务）:', { recordId })
      } else {
        console.log('复用已有生成记录:', { recordId, chatMessageId })
      }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          console.error("Kie.ai API error (webhook mode):", response.status, errorData)
        ...
        if (data.code !== 200) {
          console.error("Kie.ai API returned error (webhook mode):", data)
          return NextResponse.json(
            { error: data.msg || "AI generation failed" },
            { status: 500 }
          )
        }
        ...
        if (!taskId) {
          console.error("No taskId returned from Kie.ai (webhook mode):", data)
        ...
        try {
          ...
          console.log("生成记录已更新 taskId（Webhook模式）:", { taskId, recordId })
        } catch (updateError) {
          console.error("更新生成记录 taskId 失败（Webhook模式）:", updateError)
        }
        ...
        console.error("创建 Kie.ai 任务失败（Webhook模式）:", error)
    ...
    if (data.code !== 200) {
      ...
      return NextResponse.json({ error: data.msg || "AI generation failed" }, { status: 500 })
    }
          ...
          if (queryData.code === 200 && queryData.data) {
            ...
            if (taskData.state === 'success' && taskData.resultJson) {
              ...
              try {
                ...
                return NextResponse.json({ error: "AI generation failed - invalid result format" }, { status: 500 })
              }
            } else if (taskData.state === 'fail') {
              console.error('Kie.ai task failed:', taskData.failMsg || taskData.failCode)
              return NextResponse.json({ error: taskData.failMsg || "AI generation failed" }, { status: 500 })
            }
    ...
    if (!taskResult) {
      return NextResponse.json({ error: "AI generation timeout" }, { status: 500 })
    }
    ...
    // 扣除积分
    try {
      await usePoints(sessionUserId, pointsCost, "AI图片生成", "ai_image_generate")
    } catch (pointsError) {
    ...
    console.error("Kie.ai Generate API Error:", error)
